define(function (require) {
	...
	test('_successor() of case0', function (assert) {
		for (var i = 1; i < nodes.length - 1; i++) {
			assert.equal(case0._successor(nodes[i]), nodes[i + 1])
		}
	})

	test('_predecessor() of case0', function (assert) {
		for (var i = 1; i < nodes.length - 1; i++) {
			assert.equal(case0._predecessor(nodes[i]), nodes[i - 1])
		}
	})

	test('_minNode() of case0', function (assert) {
	...
	test('_maxNode() of case0', function (assert) {
	...
	test('min() of case0', function (assert) {
		assert.equal(case0.min(), 2)
	})

	test('max() of case0', function (assert) {
		assert.equal(case0.max(), 20)
	})

	test('count() of case0', function (assert) {
	...
	test('insertMany()', function (assert) {
		var tree = new BinarySearchTree(function (x, y) {
			return x - y
		})
